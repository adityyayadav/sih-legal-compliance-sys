...
| 2 | ML client | ✅ | **Integrated 2026-09-02** against the finished ML service (`ml-service`, commit `FINAL COMMIT FOR ML_SERVICE`). `MlAnalyzeResponse` DTO matches `POST /api/v1/analyze` exactly (`declarations` map, `font_analysis[]`, `violations[]`, `overall_compliance_status`, `confidence_flags`) — every field `@JsonProperty`-mapped. `MlServiceClient` (`@Profile("!dev & !test")`) = **WebClient multipart** (`images` file part + `product_id`), 30 s timeout. `MockMlAnalysisClient` (`@Profile("dev\|test")`) returns the same shape. `ScanService.applyReport` maps: declarations→`Declaration`, violations→FAIL/WARNING `ComplianceResult`, present-non-violating→PASS; derives PARTIAL when only MINOR issues; sets `Scan.needsManualReview`. Verified end-to-end on the dev mock; real path exercised via the same DTO but not yet against a live ML server. |
